...
interface ToDoDao {
    ...
    @Query("SELECT * FROM todo_table ORDER BY CASE WHEN priority LIKE '1%' THEN 1 WHEN priority LIKE '2%' THEN 2 WHEN priority LIKE '3%' THEN 3 END")
    fun sortByLowPriority(): Flow<List<ToDoTaskModel>>

    @Query("SELECT * FROM todo_table ORDER BY CASE WHEN priority LIKE '3%' THEN 1 WHEN priority LIKE '2%' THEN 2 WHEN priority LIKE '1%' THEN 3 END")
